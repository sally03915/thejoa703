...
import reducer, {
  ...
  toggleFollowRequest,
} from "../../reducers/followReducer"; // ✅ 경로 확인: reducers/followReducer.js
...
describe("followReducer", () => {
  ...
  // --- 언팔로우 요청 ---
  it("should handle unfollowRequest", () => {
    const state = reducer(initialState, unfollowRequest());
    expect(state.loading).toBe(true);
    expect(state.error).toBeNull();
  });
  ...
  // --- 언팔로우 실패 ---
  it("should handle unfollowFailure", () => {
    const state = reducer(initialState, unfollowFailure("언팔로우 에러"));
    expect(state.error).toBe("언팔로우 에러");
    expect(state.loading).toBe(false);
  });

  // --- 팔로워 목록 요청 ---
  it("should handle loadFollowersRequest", () => {
    const state = reducer(initialState, loadFollowersRequest());
    expect(state.loading).toBe(true);
    expect(state.error).toBeNull();
  });
  ...
  // --- 팔로워 목록 실패 ---
  it("should handle loadFollowersFailure", () => {
    const state = reducer(initialState, loadFollowersFailure("팔로워 로드 실패"));
    expect(state.error).toBe("팔로워 로드 실패");
    expect(state.loading).toBe(false);
  });

  // --- 팔로잉 목록 요청 ---
  it("should handle loadFollowingsRequest", () => {
    const state = reducer(initialState, loadFollowingsRequest());
    expect(state.loading).toBe(true);
    expect(state.error).toBeNull();
  });
  ...
  // --- 팔로잉 목록 실패 ---
  it("should handle loadFollowingsFailure", () => {
    const state = reducer(initialState, loadFollowingsFailure("팔로잉 로드 실패"));
    expect(state.error).toBe("팔로잉 로드 실패");
    expect(state.loading).toBe(false);
  });

  // --- 차단 요청 ---
  it("should handle updateBlockRequest", () => {
    const state = reducer(initialState, updateBlockRequest());
    expect(state.loading).toBe(true);
    expect(state.error).toBeNull();
  });
  ...
  // --- 차단 실패 ---
  it("should handle updateBlockFailure", () => {
    const state = reducer(initialState, updateBlockFailure("차단 실패"));
    expect(state.error).toBe("차단 실패");
    expect(state.loading).toBe(false);
  });

  // --- 팔로우 토글 (Saga 트리거용) ---
  it("should handle toggleFollowRequest (Saga trigger only)", () => {
    const state = reducer(initialState, toggleFollowRequest(99));
    // ✅ 단순히 error 초기화만 확인
    expect(state.error).toBeNull();
    // ✅ followingsMap은 건드리지 않음
    expect(state.followingsMap["99"]).toBeUndefined();
  });
});
